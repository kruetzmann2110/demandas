...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const createRankBadge = (rank) => {
        ...
        // ✅ NOVO: Extrair sigla e número (ex: Op1, Ql2, An3)
        ...
        if (match) {
            ...
            return `<span class="badge badge-rank ${rankClass}" style="background: ${color}; color: white; font-weight: bold;" title="Categoria: ${sigla} - Posição: ${numero}">${rank}</span>`;
        }
                        ...
                        <div class="card-body">
                            <form id="new-request-form">
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            <label for="demand-open-date" class="form-label text-muted"><i class="fas fa-calendar me-1"></i>Data de Abertura da Demanda</label>
                                            <input type="date" class="form-control border-light" id="demand-open-date" required>
                                            <div class="form-text">Data em que a demanda foi originalmente aberta</div>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            <label for="real-demand-id" class="form-label text-muted"><i class="fas fa-hashtag me-1"></i>ID da Demanda Real</label>
                                            <input type="text" class="form-control border-light" id="real-demand-id" placeholder="Ex: DEM-2024-001" required>
                                            <div class="form-text">Identificador original da demanda no sistema fonte</div>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="mb-3">
                            ...
                            ${userDemands.map(demand => {
                                ...
                                return `
                                    <tr>
                                        ...
                                        <td><button class="btn btn-sm btn-outline-primary" data-action="view-timeline" data-id="${demand.id}"><i class="fas fa-eye me-1"></i>Ver</button></td>
                                    </tr>
                                `;
                            }).join('')}
                        </tbody>
    ...
    const renderAnalyticsRanking = () => {
        ...
        // ✅ ESTATÍSTICAS SIMPLIFICADAS POR OPERADOR (apenas nome e aberturas)
        const userStats = {};
        allDemands.forEach(demand => {
            const requester = demand.requester || 'Não informado';
            if (!userStats[requester]) {
                userStats[requester] = { aberturas: 0 };
            }
            userStats[requester].aberturas += 1;
        });

        const operatorRows = Object.entries(userStats)
            .sort(([, a], [, b]) => b.aberturas - a.aberturas)
            .map(([user, stats]) => `
                <tr>
                    <td><strong>${getUserName(user)}</strong></td>
                    <td><span class="badge bg-primary">${stats.aberturas}</span></td>
                </tr>
            `).join('');

        // ✅ ESTATÍSTICAS POR SUBMOTIVO
        const submotivoStats = {};
        allDemands.forEach(demand => {
            const submotivo = demand.submotivo || 'Não informado';
            submotivoStats[submotivo] = (submotivoStats[submotivo] || 0) + 1;
        });

        const submotivoRows = Object.entries(submotivoStats)
            .sort(([, a], [, b]) => b - a)
            .slice(0, 10) // Top 10
            .map(([submotivo, count]) => `
                <div class="d-flex justify-content-between align-items-center mb-2 p-2 bg-light rounded">
                    <span class="text-primary fw-bold">${submotivo}</span>
        ...
        const statusRows = Object.entries(statusStats)
            .sort(([, a], [, b]) => b - a)
            .map(([status, count]) => `
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <span class="badge status-badge status-${status.toLowerCase().replace(/\s+/g, '-')}">${status}</span>
                    <span class="badge bg-secondary">${count}</span>
                </div>
            `).join('');

        // ✅ ESTATÍSTICAS POR CENÁRIO/TEMA
        const scenarioStats = {};
        allDemands.forEach(demand => {
        ...
        const scenarioRows = Object.entries(scenarioStats)
            ...
            .map(([scenario, count]) => `
                <div class="d-flex justify-content-between align-items-center mb-2 p-2 border rounded">
                    <small class="text-dark">${scenario}</small>
        ...
        const recentRows = recentDemands.map(demand => `
            <tr>
                <td><strong>#${demand.id}</strong></td>
                <td><span class="badge bg-info text-dark">${demand.real_demand_id || 'N/A'}</span></td>
                <td><small>${demand.scenario}</small></td>
                <td><small class="text-primary">${demand.submotivo || 'N/A'}</small></td>
                <td><small>${getUserName(demand.requester)}</small></td>
                <td><small>${formatDate(demand.open_date || demand.created_at)}</small></td>
            </tr>
        `).join('');
        ...
        appContent.innerHTML = `
            ...
            <div class="row mb-4">
                <div class="col-md-3">
                    <div class="analytics-card">
                        <h3 class="text-primary">${totalDemands}</h3>
                        <p><i class="fas fa-list me-1"></i>Total de Demandas</p>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="analytics-card">
                        <h3 class="text-warning">${openDemands}</h3>
                        <p><i class="fas fa-clock me-1"></i>Demandas Abertas</p>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="analytics-card">
                        <h3 class="text-success">${closedDemands}</h3>
                        <p><i class="fas fa-check-circle me-1"></i>Concluídas</p>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="analytics-card">
                        <h3 class="text-info">${Math.round((closedDemands / (totalDemands || 1)) * 100)}%</h3>
                        <p><i class="fas fa-percentage me-1"></i>Taxa de Conclusão</p>
                    </div>
                </div>
            </div>
            ...
            <!-- Dashboard Completo -->
            <div class="row mb-4">
                <!-- Estatísticas por Operador (Simplificado) -->
                <div class="col-md-4">
                    <div class="card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-users me-2"></i>Aberturas por Operador</h6>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive" style="max-height: 300px;">
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        ${operatorRows || '<tr><td colspan="2" class="text-center text-muted">Sem dados</td></tr>'}
                                    </tbody>
                                </table>
                            </div>
                        </div>
                ...
                <div class="col-md-4">
                    <div class="card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-tags me-2"></i>Top Submotivos</h6>
                        </div>
                        <div class="card-body" style="max-height: 350px; overflow-y: auto;">
                            ${submotivoRows || '<div class="text-center text-muted">Sem submotivos</div>'}
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-chart-donut me-2"></i>Status das Demandas</h6>
                        </div>
                        <div class="card-body" style="max-height: 350px; overflow-y: auto;">
                            ${statusRows}
                        </div>
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6">
                    <div class="card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-tasks me-2"></i>Top Cenários/Temas</h6>
                        </div>
                        <div class="card-body" style="max-height: 300px; overflow-y: auto;">
                            ${scenarioRows || '<div class="text-center text-muted">Sem cenários</div>'}
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="card h-100">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-clock me-2"></i>Demandas Recentes</h6>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive" style="max-height: 300px;">
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        ${recentRows || '<tr><td colspan="6" class="text-center text-muted">Sem dados</td></tr>'}
                                    </tbody>
                                </table>
                            </div>
                        </div>
            ...
            <!-- Gráfico Visual Simples de Distribuição -->
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h6 class="mb-0"><i class="fas fa-chart-bar me-2"></i>Distribuição Visual</h6>
                        </div>
                        <div class="card-body">
                            <div class="row text-center">
                                <div class="col-md-2">
                                    <div class="p-3 border rounded bg-light">
                                        <div class="h4 text-primary mb-1">${Object.keys(userStats).length}</div>
                                        <small class="text-muted">Operadores</small>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="p-3 border rounded bg-light">
                                        <div class="h4 text-info mb-1">${Object.keys(submotivoStats).length}</div>
                                        <small class="text-muted">Submotivos</small>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="p-3 border rounded bg-light">
                                        <div class="h4 text-warning mb-1">${Object.keys(scenarioStats).length}</div>
                                        <small class="text-muted">Cenários</small>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="p-3 border rounded bg-light">
                                        <div class="h4 text-success mb-1">${Object.keys(statusStats).length}</div>
                                        <small class="text-muted">Status</small>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="p-3 border rounded bg-light">
                                        <div class="h4 text-danger mb-1">${allDemands.filter(d => d.analyticsScore > 0).length}</div>
                                        <small class="text-muted">Com Pontuação</small>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="p-3 border rounded bg-light">
                                        <div class="h4 text-secondary mb-1">${allDemands.filter(d => d.real_demand_id).length}</div>
                                        <small class="text-muted">Com ID Real</small>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    const formatDate = (dateString) => {
        ...
        try {
            // ✅ CORREÇÃO: Formatar data no timezone de São Paulo
    ...
    const formatTime = (dateString) => {
        ...
        try {
            // ✅ CORREÇÃO: Formatar hora no timezone de São Paulo
            const date = new Date(dateString);
    ...
    // ✅ NOVA FUNÇÃO: Formatar data e hora completa para timeline (timezone São Paulo)
        ...
        console.log('🔐 Permissões do usuário:', permissions);
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // ✅ MAPEAR TODOS OS CAMPOS + NOVOS CAMPOS + CATEGORIA + RANKING
                    const categoryRanking = buildCategoryRankingMap();
                    
                    exportData = demands.map(demand => {
                        const categoryInfo = getCategoriaFromScenario(demand.scenario);
                        const ranking = categoryRanking.get(demand.id) || '-';
                        
                        return {
                            'ID Sistema': demand.id || '',
                            'ID Real Demanda': demand.real_demand_id || '',
                            'Submotivo': demand.submotivo || '',
                            'Categoria': demand.categoria || categoryInfo.categoria,
                            'Sigla Categoria': categoryInfo.sigla,
                            'Ranking por Categoria': ranking,
                            'Título': demand.title || '',
                            'Descrição': (demand.description || '').replace(/[\r\n]/g, ' ').replace(/,/g, ';'),
                            'Solicitante ID': demand.requester || '',
                            'Solicitante Nome': getUserName(demand.requester) || '',
                            'Responsável ID': demand.responsible || '',
                            'Responsável Nome': demand.responsible_name || getUserName(demand.responsible) || 'Não atribuído',
                            'Status Atual': demand.status || '',
                            'Prioridade': getPriorityString(demand.priority) || '',
                            'Pontuação Analytics': demand.analyticsScore || 0,
                            'Classificação Prioridade': demand.priority_classification || '',
                            'Tema/Cenário': demand.scenario || '',
                            'Grupo': demand.group_name || '',
                            'Observação Atual': (demand.observation || '').replace(/[\r\n]/g, ' ').replace(/,/g, ';'),
                            'Data Abertura Demanda': demand.open_date ? new Date(demand.open_date).toLocaleDateString('pt-BR') : '',
                            'Data Criação Sistema': demand.created_at ? new Date(demand.created_at).toLocaleString('pt-BR') : '',
                            'Data Atualização': demand.updated_at ? new Date(demand.updated_at).toLocaleString('pt-BR') : '',
                            'Data Prazo': demand.deadline ? new Date(demand.deadline).toLocaleDateString('pt-BR') : '',
                            'Aging (dias)': demand.open_date ? Math.ceil((new Date() - new Date(demand.open_date)) / (1000 * 60 * 60 * 24)) : '',
                            'Último Status Timeline': demand.ultimo_status_timeline || 'Nenhum status alterado',
                            'Data Último Status': demand.ultimo_status_data ? new Date(demand.ultimo_status_data).toLocaleString('pt-BR') : '',
                            'Última Observação Timeline': demand.ultima_observacao_timeline || 'Nenhuma observação',
                            'Data Última Observação': demand.ultima_observacao_data ? new Date(demand.ultima_observacao_data).toLocaleString('pt-BR') : ''
                        };
                    });
                ...
                exportData = localDemands.map(demand => ({
                    ...
                    'Descrição': (demand.description || '').replace(/[\r\n]/g, ' ').replace(/,/g, ';'),
                    'Solicitante ID': demand.requester || '',
                    'Solicitante Nome': demand.requester_name || '',
                    'Responsável ID': demand.responsible || '',
                    'Responsável Nome': demand.responsible_name || 'Não atribuído',
                    'Status Atual': demand.status || '',
                    'Prioridade': demand.priority || '',
                    'Tema': demand.theme || '',
                    'Grupo': demand.group_name || '',
                    'Cenário': demand.scenario || '',
                    'Observação Atual': demand.observation || '',
                    'Pontuação Analytics': demand.analyticsScore || '',
                    'Data Criação': demand.created_at ? new Date(demand.created_at).toLocaleString('pt-BR') : '',
                    'Data Atualização': demand.updated_at ? new Date(demand.updated_at).toLocaleString('pt-BR') : '',
                    'Data Prazo': demand.deadline ? new Date(demand.deadline).toLocaleString('pt-BR') : '',
                    'Último Status Timeline': 'Dados locais - sem timeline',
                    'Data Último Status': '',
                    'Última Observação Timeline': 'Dados locais - sem timeline',
                    'Data Última Observação': ''
                }));
            }
    ...
    window.viewTimeline = async (demandId) => {
        try {
            ...
            console.log('📅 Carregando timeline da demanda', demandId);
            ...
            if (demand.timeline && Array.isArray(demand.timeline) && demand.timeline.length > 0) {
                ...
                console.log(`✅ Timeline do cache: ${timeline.length} eventos`);
            } else {
                console.log('⚠️ Timeline não encontrada no cache, buscando via API...');
                try {
                    // Buscar timeline específica da demanda via API
                    const timelineResult = await apiRequest(`/demands/${demandId}/timeline`);
                    console.log('📡 Resposta da API:', timelineResult);
                    ...
                    if (timelineResult && timelineResult.success && timelineResult.data) {
                        ...
                        timeline = timelineResult.data.map(event => ({
                            id: event.id,
                            demand_id: event.demand_id,
                            usuario: 'Sistema', // Campo não existe na tabela real, usar padrão
                            acao: event.event_text, // Usar event_text real da tabela
                            descricao: event.event_text, // Usar event_text como descrição
                            data_acao: event.event_date, // Usar event_date real da tabela
                            created_at: event.created_at // Usar created_at real da tabela
                        }));
                        console.log(`✅ Timeline carregada via API: ${timeline.length} eventos`);
                    } else {
                        console.log('⚠️ Timeline não encontrada via API, criando entrada inicial...');
                        timeline = [{
                            usuario: 'Sistema',
                            acao: 'Demanda Criada',
                    ...
                    console.error('❌ Erro ao carregar timeline via API:', error);
            ...
            console.log('✅ Timeline preparada:', timeline.length, 'eventos');
        ...
        console.log('🔐 Verificando permissões da timeline:');
        console.log(`  - Usuário atual: ${currentUser}`);
        ...
        try {
            ...
            if (el) { el.style.display = 'block'; el.classList.add('show'); el.removeAttribute('aria-hidden'); }
        }
        ...
        startAutoRefresh();
            ...
            showToast('Erro ao carregar timeline da demanda', 'error');
        }
    };
